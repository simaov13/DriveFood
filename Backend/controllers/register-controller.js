...
exports.register = (req, res) => {
    try {
        ...
        if (username.length < 5) {
            res.status(411).send({ message: 'O nome de utilizador tem de ter 5 ou mais caracteres' });
            throw "O nome de utilizador tem de ter 5 ou mais caracteres";
        }
        //Palavra passe maior que 8 caracteres
        if (password.length < 8) {
            res.status(411).send({ message: 'A palavra-passe tem de ter 8 ou mais caracteres' });
        ...
        if (type == 'user' || type == 'merchant') {
            ...
            if (!Number.isInteger(nif) && nif.length != 9) {
                res.status(406).send({ message: 'NIF inválido' });
                throw "Nif inválido";
            }
        }
        //Não pode ser for diferente de 9
        if (!Number.isInteger(phone) && phone.length != 9) {
            res.status(406).send({ message: 'Telemovel inválido' });
            throw "Telemovel inválido";
        }
        //Diferente de algum utilizador
        if (type != 'user' && type != 'driver' && type != 'merchant') {
            res.status(406).send({ message: 'Tipo de utilizador inválido ("user"/"driver"/"merchant"/"admin")' });
